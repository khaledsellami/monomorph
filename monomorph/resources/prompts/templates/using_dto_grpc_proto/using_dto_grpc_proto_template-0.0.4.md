...
{% if methods|length == 0 %}
The following class `{{ class_.full_name }}` is referenced by external microservices but does not expose any methods. Therefore, this proto file will reprensent its equivalent Data Transfer Object (DTO) only.
{% else %}
...
You are not able to change the original class. You will be given the source code of the class in question and the instructions to create the necessary files. 
...
- Creates a message that represents the Data Transfer Object (DTO) for the class `{{ class_.name }}`. The message should be named `{{ class_.name }}DTO`
{% if fields|length > 0 %}
- The DTO should contain at least the following fields from the class: [{% for field in fields %}`{{ field }}`,  {% endfor %}] and any other fields that are necessary to represent the class.
- The fields must retain the same names as in the original source code even if they do not follow the protobuf naming conventions.{% if id_refs or dto_refs %} The exceptions are the fields that are mapped to other {{ refs_text }} classes (see below for details).{% endif %}
{% endif %}
{% if methods %}
- It exposes only the used methods: [{% for method in methods %}`{{ method }}`, {% endfor %}]
