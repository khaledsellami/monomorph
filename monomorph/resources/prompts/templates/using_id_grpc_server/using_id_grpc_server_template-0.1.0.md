...
- Some method inputs and outputs have been changed to `RefactoredObjectID` in the proto file. This ID can be used to retrieve or create the proxies that correspond to these classes. 
  - Each proxy will have the same name as the class it represents. 
  - Each proxy has a static method `fromID` that takes a `RefactoredObjectID` and returns the corresponding proxy instance. 
  - Each proxy has a method `toID` that takes the original class instance and returns the corresponding `RefactoredObjectID`. 
  - However, if the `RefactoredObjectID` must be mapped from or to the actual class instance, the same logic as the constructor should be used. To do this, you can use the `{{ mapper_class.full_name }}` utility class shown below.
{% endif %}
